...
function setup_connect(button_svg, button_color) {
    ...
    console.log('VehicleManager laoded', window.createVehicleParser)
    console.log('Parser map', window.MAVLinkParsers)
    ...
    //IB Add vehicle button
    const vehicleMap = new Map();
    let activeVehicle = null;
    let vehicleCount = 0;
    const add_button = tip_div.querySelector('input[id="add_button"]')
    const form1 = tip_div.querySelector('div[id="form1"]')
    //const newInput = new inputters(addVehicle(), addName());


    //IB Creates new websocket input
    function addVehicle() {
        const ipURL = document.createElement('input');
        ipURL.id = 'target_url' + vehicleCount;
        ipURL.type = 'url';
        ipURL.placeholder = 'ws://127.0.0.1:' + (56781 + vehicleCount);
        ipURL.required = 'true';
        ipURL.pattern = '^(ws|wss)://.*';
        return ipURL;
    }

    //IB Creates complimentary name input
    function addName() {
        const ipName = document.createElement('input');
        ipName.id = 'name' + vehicleCount;
        ipName.placeholder = 'Unique Vehicle Name';
        ipName.required = 'true';
        return ipName;
    }

    //IB on add button click creates object newInput, 
    add_button.onclick = () => {
        ...
        form1.appendChild(newVehicle);
        form1.appendChild(newName);

        const id = crypto.randomUUID();
        const vehicle = new inputters(newVehicle, newName, id);
        const vehicleParsed = window.createVehicleParser(vehicle, id);
        vehicleMap.set(id, vehicle);
        activeVehicle = vehicle;
        //vehicleName = tip_div.querySelector('input[id="target_url"]');
        //const veh = new MavVehicle(vehicleName, vehicleCount);
        //veh.display();
    }
    ...
    //this bit probably needs editing
    function set_inputs(connected) {
        ...
        //vehicle.url_input.disabled = connected
    ...
    function connect(vehicle, auto_connect) { //IB add target1
        ...
        if (!vehicle.ws) {
            console.error('veh or ws not ready', vehicle)
            return
        }
        ...
        vehicle.ws.addEventListener('open', (e) => {
            console.error('err', e);
            console.log('we have open!');
        });

        vehicle.ws.onclose = (e) => {
        ...
        //ws.onerror moved inside vehicle

        

        vehicle.ws.onmessage = (msg) => {
            if (msg === undefined) {
                console.log('ahhhhh')
            }
            else {
            ...
            console.log('ms', msg === undefined)
            //console.log('Messages');
            console.log('type'+ typeof msg.data);
            console.log('array'+ msg.data instanceof ArrayBuffer);
            console.log('blob'+ msg.data instanceof Blob);
            console.log('uint8arry'+ msg.data instanceof Uint8Array);
        
            // Feed data to MAVLink parser and forward messages
            const veh = window.MAVLinkParsers.get(vehicle.id)
            if (!veh) {
                console.log('did not get the message');
                return
            }
            for (const char of new Uint8Array(msg.data)) {
                const m = veh.parser.parseChar(char)
                if ((m != null) && (m._id != -1)) {
        ...
        //ws1.onmessage = (msg1) => {
            // Feed data to MAVLink parser and forward messages
        //    for (const char1 of new Uint8Array(msg1.data)) {
        //        const m1 = MAVLink1.parseChar(char1)
        //        if ((m1 != null) && (m1._id != -1)) {
        //            m1._timeStamp = Date.now()
        //            broadcast.postMessage({ MAVLink1: m1 })
        //        }
        //    }
        //}
    ...
    function disconnect() {
        ...
        button_color("black")
        activeVehicle.target.disabled = false
    ...
    connect_button.onclick = () => {
        ...
        // Checks for active vehicle IB
        if (!activeVehicle) {
            console.error('No active vehicle');
            return;
        }

        // Sets WebSocket to value inputted IB
        activeVehicle.set_ws();
        ...
        if (!activeVehicle.target.checkValidity()) {
            // Invalid address, re-fire the tip and focus the url
            tip.show()
            activeVehicle.target.focus()
            return
        }
        ...
        activeVehicle.target.disabled = true
        ...
        console.log(activeVehicle.target)
        connect(activeVehicle) //IB add url_unput1.value
