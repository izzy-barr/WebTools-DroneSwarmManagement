...
function vehicle_init(id, location, type, colour) {
    ...
    marker.on("click", (e) => {
        ...
        fill_vehicle_info(clickedVehicle) //IB
        window.tip.show() //IB
    })
...
function init_vehicle_info() {
    ...
    tip_div.style.height = "100vh"
    tip_div.style.maxHeight = "100vh"
    tip_div.style.overflow = "hidden"
    tip_div.style.boxSizing = "border-box"
    ...
    tip_div.querySelector(`input[id="vehicle_colour"]`).onchange = () => {
        parent.vehicleMap.get(clickedVehicle).colour = tip_div.querySelector(`input[id="vehicle_colour"]`).value
        console.log('Vehicle colour changed to',  parent.vehicleMap.get(clickedVehicle).colour)
    }
...
function fill_vehicle_info() {
    tip_div.querySelector(`b[id="vehicle_name"]`).innerHTML = parent.vehicleMap.get(clickedVehicle).name
    tip_div.querySelector(`b[id="vehicle_ws"]`).innerHTML = parent.vehicleMap.get(clickedVehicle).target 
    tip_div.querySelector(`input[id="vehicle_colour"]`).value = parent.vehicleMap.get(clickedVehicle).colour 
...
let comp_id = {} //IB move here
const tree_div = document.createElement("div") //IB move back here
let ids = {}//IB add here
